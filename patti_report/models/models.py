...
class PattiReportWizard(models.TransientModel):
    ...
    def generate_xlsx_report(self):
        ...
        if not report_log:
            ...
            # Report Title
            sheet.merge_range('A1:T1', 'Patti Report', title_format)
            sheet.merge_range('A2:T2',
            ...
            sheet.write('H3', 'Cutting Bages', title_format)
            sheet.write('I3', 'Net Weight', title_format)
            sheet.write('J3', 'Moisture KG', title_format)
            sheet.write('K3', 'Moisture Cut', title_format)
            sheet.write('L3', 'Qtls Weight', title_format)
            sheet.write('M3', 'Shortage Weight', title_format)
            sheet.write('N3', 'Quantity', title_format)
            sheet.write('O3', 'Rate', title_format)
            sheet.write('P3', 'Gross', title_format)
            sheet.write('Q3', 'Rusum', title_format)
            sheet.write('R3', 'Freight', title_format)
            sheet.write('S3', 'Other Debits & Credits', title_format)
            sheet.write('T3', 'Net Amount', title_format)
            ...
            # cutting_bags = '=SUM(H4:H{})'.format(row)
            sheet.write(row, 7, '', border)
            net_wt = '=SUM(I4:I{})'.format(row)
            sheet.write(row, 8, net_wt, border)
            # moisture_kg ='=SUM(J4:J{})'.format(row)
            sheet.write(row, 9, '', border)
            moisture_cut = '=SUM(K4:K{})'.format(row)
            sheet.write(row, 10, moisture_cut, border)
            # qtls = '=SUM(L4:L{})'.format(row)
            sheet.write(row, 11, '', border)
            shortage_wt = '=SUM(M4:M{})'.format(row)
            sheet.write(row, 12, shortage_wt, border)
            qty = '=SUM(N4:N{})'.format(row)
            sheet.write(row, 13, qty, border)
            sheet.write(row, 14, '', border)
            # gross = '=SUM(P4:P{})'.format(row)
            sheet.write(row, 15, '', border)
            rusum = '=SUM(Q4:Q{})'.format(row)
            sheet.write(row, 16, rusum, border)
            freight = '=SUM(R4:R{})'.format(row)
            sheet.write(row, 17, freight, border)
            # othdc = '=SUM(S4:S{})'.format(row)
            sheet.write(row, 18, '', border)
            nt_amt = '=SUM(T4:T{})'.format(row)
            sheet.write(row, 19, nt_amt, border)
